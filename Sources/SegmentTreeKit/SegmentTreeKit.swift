...
public struct SegmentTree<Node: STNode>: Collection {
    ...
    public init(
        capacity size: Int, of nodeType: Node.Type = Node.self, fill: Node = Node(),
        updateType: STUpdateType = .immediate
    ) {
        self.offset =
            if size.nonzeroBitCount == 1 { size } else {
                1 << (Int.bitWidth - size.leadingZeroBitCount)
            }
    ...
    private func query(index: Int, bounds: Range<Int>, queryBounds: Range<Int>) -> Node? {
        if queryBounds.contains(bounds) {
            return data[index]
        } else if !queryBounds.overlaps(bounds) {
            return nil
        } else {
            let mid = bounds.lowerBound + ((bounds.upperBound - bounds.lowerBound) / 2)
            let lhs = query(
                index: index * 2, bounds: bounds.lowerBound..<mid, queryBounds: queryBounds)
            let rhs = query(
                index: index * 2 + 1, bounds: mid..<bounds.upperBound, queryBounds: queryBounds)
    ...
    public subscript(bounds: Range<Int>) -> Node {
        ...
        return query(index: 1, bounds: 0..<self.offset, queryBounds: bounds)
            !! "Query on valid range must always return a `Node`"
    }

    public subscript(bounds: ClosedRange<Int>) -> Node {
        precondition(self.bounds.contains(bounds), "Range Query on invalid range!")
        return query(
            index: 1, bounds: 0..<self.offset,
            queryBounds: bounds.lowerBound..<bounds.upperBound + 1)
            !! "Query on valid range must always return a `Node`"
    }
